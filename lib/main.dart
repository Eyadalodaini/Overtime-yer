...
String money(dynamic amount) {
  final val = amount is num ? amount : double.tryParse(amount.toString()) ?? 0;
  return '${val.toStringAsFixed(0)} YER';
}
...
class Settings {
  ...
  Future<void> load() async {
    ...
    for (final entry in endTimes.entries) {
      endTimes[entry.key] = _prefs!.getString('end_${entry.key}') ?? entry.value;
    }
  }
...
class AppDb {
  ...
  Future<int> insert(Map<String, dynamic> row) => _db!.insert('overtime', row);
...
class NotificationService {
  ...
  static Future<void> schedule({
    ...
    for (final weekday in reminderWeekdays) {
      ...
      final daysAhead =
          (weekday - next.weekday + DateTime.daysPerWeek) % DateTime.daysPerWeek;
...
class _HomePageState extends State<HomePage> {
  ...
  int get totalMoney => rows.fold<int>(0, (s, r) => s + (r['total'] as int));
          ...
          if (rows.isEmpty)
            const Card(
              child: Padding(
                ...
                child: Center(
                    child: Text('لا توجد سجلات بعد. أضف أول ساعاتك الإضافية.')),
              ),
            )
          else
        ...
        trailing: PopupMenuButton<String>(
          onSelected: (v) async {
            if (v == 'edit') {
              await showDialog(
                context: context,
                builder: (_) => AddDialog(existing: row, onSaved: onRefresh),
              );
            } else if (v == 'delete') {
...
class _AddDialogState extends State<AddDialog> {
  ...
  Future<void> save() async {
    final h =
        useTimes ? _calculatedHours() : (double.tryParse(hours.text.trim()) ?? 0);
            ...
            children: [
              ...
              if (useTimes) ...[
                ListTile(
                  contentPadding: EdgeInsets.zero,
                  title: Text(
                      'بداية الإضافي: ${start == null ? 'اختر الوقت' : start!.format(context)}'),
                  onTap: () => pickTime(true),
                ),
                ListTile(
                  contentPadding: EdgeInsets.zero,
                  title: Text(
                      'نهاية الإضافي: ${end == null ? 'اختر الوقت' : end!.format(context)}'),
              ...
              if (useTimes && start != null && end != null)
                Text(
                    'الإجمالي المتوقع: ${money(_calculatedHours() * Settings.instance.rate)}'),
            ],
          ),
        ),
        actions: [
            ...
            children: [
              Expanded(
                child: OutlinedButton(
                  onPressed: () => pick(true),
                  child: Text('من ${fmtDate(DateFormat('yyyy-MM-dd').format(from))}'),
                ),
              ),
              ...
              Expanded(
                child: OutlinedButton(
                  ...
                  child: Text('إلى ${fmtDate(DateFormat('yyyy-MM-dd').format(to))}'),
                ),
              ),
            ],
          ),
        Card(
          child: Padding(
            ...
            child: Column(
              children: [
                Text(
                    'الفترة: ${fmtDate(DateFormat('yyyy-MM-dd').format(from))} — ${fmtDate(DateFormat('yyyy-MM-dd').format(to))}'),
                const SizedBox(height: 16),
                Text('$h ساعة',
                    style: Theme.of(context).textTheme.headlineMedium),
                Text(money(moneyTotal),
      ...
      children: [
        TextField(
          ...
          decoration: const InputDecoration(
            labelText: 'سعر الساعة (YER)',
            border: OutlineInputBorder(),
          ),
          onChanged: (v) {
            final val = int.tryParse(v);
            if (val != null && val > 0) {
              Settings.instance.rate = val;
