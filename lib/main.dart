  ...
  Future<void> replaceAll(List<dynamic> rows) async {
    await _db!.transaction((txn) async {
      ...
      for (final item in rows) {
        if (item is! Map) {
          throw const FormatException('سجل احتياطي غير صالح');
        }
        ...
        if (date is! String ||
            ...
            rate <= 0 ||
            total is! num) {
          throw const FormatException('بيانات سجل غير صالحة');
        }

        await txn.insert('overtime', {
          'date': date.substring(0, 10),
          'start': item['start'] is String ? item['start'] : null,
          'end': item['end'] is String ? item['end'] : null,
          'hours': hours.toDouble(),
          'rate': rate.round(),
          'total': total.round(),
          'note': item['note'] is String ? item['note'] : null,
        });
      }
    });
  }
...
class NotificationService {
  ...
  // أيام العمل: السبت إلى الخميس، والجمعة إجازة.
  ...
  static Future<void> cancel() async {
    for (final weekday in reminderWeekdays) {
      await _plugin.cancel(1000 + weekday);
    }
  }
  ...
  static Future<void> schedule({
    ...
    for (final weekday in reminderWeekdays) {
      ...
      final daysAhead = (weekday - next.weekday + DateTime.daysPerWeek) %
...
class _AddDialogState extends State<AddDialog> {
  ...
  Future<void> save() async {
    ...
    if (!h.isFinite || h <= 0 || h > 24) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('عدد الساعات يجب أن يكون بين 0 و24')),
        );
      }
      return;
    }
...
class _SettingsPageState extends State<SettingsPage> {
  ...
  Future<void> saveRate() async {
    final value = int.tryParse(rate.text.trim());
    if (value == null || value <= 0) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('أدخل سعر ساعة صحيحًا أكبر من صفر')),
        );
      }
      return;
    }

    Settings.instance.rate = value;
    await Settings.instance.save();
    SettingsNotifier.instance.refresh();
    if (mounted) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('تم حفظ سعر الساعة')),
      );
      setState(() {});
    }
  }
  ...
  Future<void> restore() async {
    ...
    try {
      ...
      if (s['rate'] is num && (s['rate'] as num) > 0) {
        Settings.instance.rate = (s['rate'] as num).round();
      }
      if (s['colorSeed'] is num) {
        Settings.instance.colorSeed =
            (s['colorSeed'] as num).round().clamp(0, 4);
      }
      if (s['reminders'] is bool) {
        Settings.instance.reminders = s['reminders'] as bool;
      }
      if (s['reminderHour'] is num) {
        Settings.instance.reminderHour =
            (s['reminderHour'] as num).round().clamp(0, 23);
      }
      if (s['reminderMinute'] is num) {
        Settings.instance.reminderMinute =
            (s['reminderMinute'] as num).round().clamp(0, 59);
      }
      final mode = s['themeMode'] as String?;
      if (mode != null) {
        Settings.instance.themeMode = ThemeMode.values.firstWhere(
          (e) => e.name == mode,
          orElse: () => ThemeMode.system,
        );
      }
      final savedEndTimes = s['endTimes'];
      if (savedEndTimes is Map) {
        for (final key in Settings.instance.endTimes.keys) {
          final value = savedEndTimes['$key'];
          if (value is String && RegExp(r'^([01]\\d|2[0-3]):[0-5]\\d$').hasMatch(value)) {
            Settings.instance.endTimes[key] = value;
          }
        }
      }
      await Settings.instance.save();
    ...
    return ListView(
      ...
      children: [
        ...
        Card(child: SwitchListTile(
          ...
          onChanged: (v) async { Settings.instance.reminders = v; await Settings.instance.save(); await NotificationService.schedule(
        enabled: Settings.instance.reminders,
        hour: Settings.instance.reminderHour,
        minute: Settings.instance.reminderMinute,
      ); setState(() {}); },
